...
echo "=== 3. Fixing Univalue (The Critical Patch) ==="
...
echo "=== 4. Patching Linker Dependencies (Fixing Undefined References) ==="
# Add missing libraries to all executables
...
echo "=== 5. Manually Building Submodules (Bypassing Recursion Errors) ==="
# We enter the submodules and force a clean build. This updates their timestamps so the main build won't try to regenerate them.
...
# We use -k (keep going) to ensure it doesn't stop on minor warnings
make -j$(nproc)

echo "=== DONE. If successful, binaries are in src/ and qt/ ==="
